...
class DAN_PLUS:
    ...
    def convlayers(self):
        ...
        with tf.name_scope("conv1_1") as scope:
            kernel = tf.Variable(
                tf.compat.v1.truncated_normal([3, 3, 3, 64], dtype=tf.float32, stddev=1e-1),
                name="weights",
            )
        ...
        with tf.name_scope("conv1_2") as scope:
            kernel = tf.Variable(
                tf.compat.v1.truncated_normal([3, 3, 64, 64], dtype=tf.float32, stddev=1e-1),
                name="weights",
            )
        ...
        with tf.name_scope("conv2_1") as scope:
            kernel = tf.Variable(
                tf.compat.v1.truncated_normal([3, 3, 64, 128], dtype=tf.float32, stddev=1e-1),
                name="weights",
            )
        ...
        with tf.name_scope("conv2_2") as scope:
            kernel = tf.Variable(
                tf.compat.v1.truncated_normal([3, 3, 128, 128], dtype=tf.float32, stddev=1e-1),
                name="weights",
            )
        ...
        with tf.name_scope("conv3_1") as scope:
            kernel = tf.Variable(
                tf.compat.v1.truncated_normal([3, 3, 128, 256], dtype=tf.float32, stddev=1e-1),
                name="weights",
            )
        ...
        with tf.name_scope("conv3_2") as scope:
            kernel = tf.Variable(
                tf.compat.v1.truncated_normal([3, 3, 256, 256], dtype=tf.float32, stddev=1e-1),
                name="weights",
            )
        ...
        with tf.name_scope("conv4_1") as scope:
            kernel = tf.Variable(
                tf.compat.v1.truncated_normal([3, 3, 256, 512], dtype=tf.float32, stddev=1e-1),
                name="weights",
            )
        ...
        with tf.name_scope("conv4_2") as scope:
            kernel = tf.Variable(
                tf.compat.v1.truncated_normal([3, 3, 512, 512], dtype=tf.float32, stddev=1e-1),
                name="weights",
            )
    ...
    def dan_part(self):
        ...
        with tf.name_scope("reg_head") as scope:
            shape = 4 * int(np.prod(self.maxpool5_3.get_shape()[1:]))
            print(shape)
            
            fc1w = tf.Variable(
                tf.compat.v1.truncated_normal([shape, 5], dtype=tf.float32, stddev=1e-1),
                name="weights",
            )
            fc1b = tf.Variable(
            ...
            maxpool5_2_flat = tf.nn.l2_normalize(
                tf.reshape(self.maxpool5_2, [-1, int(shape / 4)]), 1
            )
            avgpool5_2_flat = tf.nn.l2_normalize(
                tf.reshape(self.avgpool5_2, [-1, int(shape / 4)]), 1
            )

            maxpool5_3_flat = tf.nn.l2_normalize(
                tf.reshape(self.maxpool5_3, [-1, int(shape / 4)]), 1
            )
            avgpool5_3_flat = tf.nn.l2_normalize(
                tf.reshape(self.avgpool5_3, [-1, int(shape / 4)]), 1
            )
