...
LEARNING_RATES = [0.01, 0.005, 0.0005, 0.0001]  # 学习率
BOUNDARIES = [8000, 18000, 28000]  # 学习衰减轮数   NUM_TRAIN_VIDEOS*N_EPOCHS/BATCH_SIZE=4500*5=22500
BATCH_SIZE = 32
N_EPOCHS = 4
REG_PENALTY = 0
NUM_IMAGES = 280000
...
with tf.compat.v1.Session(config=config) as sess:
    ...
    tr_filename_queue = tf.compat.v1.train.string_input_producer(
        ["Dan_image_train.tfrecords"], num_epochs=2 * N_EPOCHS
    )
    ...
    val_filename_queue = tf.compat.v1.train.string_input_producer(
        ["Dan_image_val.tfrecords"], num_epochs=N_EPOCHS
    )
    ...
    for epoch in range(N_EPOCHS):
        ...
        # 一次epoch结束，把训练完的参数保存
        with open("param/param_dl_imgs" + str(param_num) + ".pkl", "wb") as pfile:
    ...
    saver.save(sess, "model_dan/model_image.ckpt")
    print("Session Saved!!")
    with open("loss_dan_image_full.pkl", "wb") as pfile:
        pickle.dump(loss_list, pfile, pickle.HIGHEST_PROTOCOL)
    print("Loss_dan_image List Saved!!")
