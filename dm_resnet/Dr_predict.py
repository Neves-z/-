...
def data_process(file_name):
    ...
    try:
        os.makedirs("predict/dan_res")
    except OSError:
        print("Error: Creating predict/dan_res")

    predict_filename = "predict/dan_res/predict_dan_res.tfrecords"  # address to save the TFRecords file
...
def predict(file_name):
    ...
    with tf.compat.v1.Session(config=config) as sess:
        ...
        pre_filename_queue = tf.compat.v1.train.string_input_producer(
            ["predict/dan_res/predict_dan_res.tfrecords"], num_epochs=1
        )
        ...
        file_list = ["param/param_dr_imgs3.pkl", "param/param_dr_ados3.pkl"]
        model.load_trained_model(file_list[0], sess, is_imgs=True)
        model.load_trained_model(file_list[1], sess, is_imgs=False)
...
def Dr_result(video_name, value):
    ...
    acc = 1 - np.absolute(output - value_1)
