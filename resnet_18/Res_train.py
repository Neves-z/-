...
BOUNDARIES = [5000, 15000, 24000, 31000]  # 学习衰减轮数   NUM_TRAIN_VIDEOS*N_EPOCHS/BATCH_SIZE=9000*4=36000
...
NUM_TRAIN_VIDEOS = 288000
NUM_VAL_VIDEOS = 64000
...
with tf.compat.v1.Session(config=config) as sess:
    model = Resnet_18(imgs, ados, REG_PENALTY=REG_PENALTY, is_training=True)
    output = model.output
    ...
    tr_filename_queue = tf.compat.v1.train.string_input_producer(
        ['Res_train.tfrecords'], num_epochs=2 * N_EPOCHS
    )
    #
    ...
    for epoch in range(N_EPOCHS):
        ...
        while i < NUM_TRAIN_VIDEOS:
            ...
            if not i % 3200:
                # 训练完成的图片的百分比
                per = float(i) / NUM_TRAIN_VIDEOS * 100
                print(
                    "Epoch:"
                ...
                ftime = time.time()
                # 计算预计训练完的剩余时间
                remtime = (ftime - stime) * ((NUM_TRAIN_VIDEOS - i) / 3200)
                stime = ftime
                # 转化为时分秒
                printTime(remtime)

        # 一次epoch结束，把训练完的参数保存
        with open("param/param_resnet" + str(param_num) + ".pkl", "wb") as pfile:
    ...
    save_path = saver.save(sess, "model_res/model_resnet.ckpt")
    print("Session Saved!! The path is: ",save_path)
    with open("loss_res_full.pkl", "wb") as pfile:
        pickle.dump(loss_list, pfile, pickle.HIGHEST_PROTOCOL)
    print("Loss_res List Saved!!")
