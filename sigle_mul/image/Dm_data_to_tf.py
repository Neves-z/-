...
def test_data_to_TF(per_video=48):
    ...
    c = list(zip(image_addrs, labels))  # zip函数作用：依次取一个元组，组成一个元组  即将图片和标签一一对应打包
    shuffle(c)  # 将序列中的元素随机排序防止一个视频的n个照片在一起
    train_image_addrs, train_labels = zip(*c)  # 再把图片、音频和标注分别取出保存


    train_filename = "Dan_max_train.tfrecords"  # address to save the TFRecords file
    ...
    for i in range(len(train_image_addrs)):  # 把图片和对应标注一次写入tfrecords文件
        # 每写入两千条 打印一次
        if not i % 2000:
            print("Train data of epoch: {}/{} ".format(i, len(train_labels)))
            sys.stdout.flush()
        # Load the image  加载图片
...
def val_data_to_TF(per_video=32):
    ...
    c = list(zip(image_addrs, labels))  # zip函数作用：依次取一个元组，组成一个元组  即将图片和标签一一对应打包
    shuffle(c)  # 将序列中的元素随机排序防止一个视频的n个照片在一起
    val_image_addrs, val_labels = zip(*c)  # 再把图片、音频和标注分别取出保存

    val_filename = "Dan_max_val.tfrecords"  # address to save the TFRecords file
    # open the TFRecords file
    ...
    for i in range(len(val_image_addrs)):  # 把图片和对应标注一次写入tfrecords文件
        # 每写入两千条 打印一次
        if not i % 2000:
            print("Validation data of epoch: {}/{} ".format(i, len(val_labels)))
            sys.stdout.flush()
        # Load the image  加载图片
