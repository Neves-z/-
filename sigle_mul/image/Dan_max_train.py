...
with tf.compat.v1.Session(config=config) as sess:
    ...
    tr_filename_queue = tf.compat.v1.train.string_input_producer(
        ["Dan_max_train.tfrecords"], num_epochs=2 * N_EPOCHS
    )
    ...
    val_filename_queue = tf.compat.v1.train.string_input_producer(
        ["Dan_max_val.tfrecords"], num_epochs=N_EPOCHS
    )
    ...
    for epoch in range(N_EPOCHS):
        ...
        # 一次epoch结束，把训练完的参数保存
        with open("param/param_dan_max" + str(param_num) + ".pkl", "wb") as pfile:
    ...
    saver.save(sess, "model_dan_max/model_image.ckpt")
    print("Session Saved!!")
    with open("loss_dan_max_img.pkl", "wb") as pfile:
        pickle.dump(loss_list, pfile, pickle.HIGHEST_PROTOCOL)
    print("Loss_dan_max List Saved!!")
