...
from Dr_net import net
from remtime import *

warnings.filterwarnings("ignore")

LEARNING_RATES = [0.01, 0.005, 0.001, 0.0005, 0.0001]  # 学习率
BOUNDARIES = [10000, 20000, 30000, 45000]  # 学习衰减轮数   NUM_TRAIN_VIDEOS*N_EPOCHS/BATCH_SIZE=9000*6=54000
...
with tf.compat.v1.Session(config=config) as sess:
    ...
    model = net(imgs, ados, REG_PENALTY=REG_PENALTY, is_training=True)
    output = model.output
    ...
    tr_filename_queue = tf.compat.v1.train.string_input_producer(
        ['Dr_train.tfrecords'], num_epochs=2 * N_EPOCHS
    )
    ...
    tr_images, tr_audios, tr_labels = tf.compat.v1.train.shuffle_batch(
        [tr_image, tr_audio, tr_label],
    ...
    val_filename_queue = tf.compat.v1.train.string_input_producer(
        ["Dr_val.tfrecords"], num_epochs=N_EPOCHS
    )
    ...
    # 用vgg-19模型初始化模型
    model.initialize_with_vgg_19("vgg-19.mat", sess)
    ...
    for epoch in range(N_EPOCHS):
        ...
        while i < NUM_IMAGES:
            ...
            try:
                ...
                epoch_x, epoch_y, epoch_z = sess.run([tr_images, tr_audios, tr_labels])
            except:
            ...
            _, c = sess.run(
                [optimizer, cost],
                feed_dict={imgs: epoch_x.astype(np.float32), ados: epoch_y.astype(np.float32), values: epoch_z},
            )
            ...
            if not i % 3200: #2000
                ...
                ftime = time.time()
                # 计算预计训练完的剩余时间
                remtime = (ftime - stime) * ((NUM_IMAGES - i) / 3200)
                stime = ftime
                # 转化为时分秒
                printTime(remtime)
        ...
        # 一次epoch结束，把训练完的参数保存
        with open("param/param_dr_imgs" + str(param_imgs_num) + ".pkl", "wb") as pfile:
            pickle.dump(sess.run(model.parameters), pfile, pickle.HIGHEST_PROTOCOL)
            print("images: " + str(param_imgs_num) + " weights Saved!!")
            param_imgs_num += 1
        with open("param/param_dr_ados" + str(param_ados_num) + ".pkl", "wb") as pfile:
            pickle.dump(sess.run(model.ado_parameters), pfile, pickle.HIGHEST_PROTOCOL)
            print("audio: " + str(param_ados_num) + " weights Saved!!")
            param_ados_num += 1
        ...
        while i < NUM_IMAGES:
            ...
            tr_acc = 1 - np.absolute(output - epoch_z)
    ...
    saver.save(sess, "model_dan_res/model.ckpt")
    print("Session Saved!!")
    with open("loss_dan_res.pkl", "wb") as pfile:
        pickle.dump(loss_list, pfile, pickle.HIGHEST_PROTOCOL)
    print("Loss_dan_res List Saved!!")
