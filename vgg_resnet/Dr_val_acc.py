...
def predict_val(NUM_VAL,PER_VAL):
    ...
    # 加载训练好的参数
    image_file_list = ["param/param_dr_imgs1.pkl",
                       "param/param_dr_imgs2.pkl",
                       "param/param_dr_imgs3.pkl",
                       "param/param_dr_imgs4.pkl",
                       ]
    audio_file_list = ["param/param_dr_ados1.pkl",
                       "param/param_dr_ados2.pkl",
                       "param/param_dr_ados3.pkl",
                       "param/param_dr_ados4.pkl",
                       ]
    ...
    with tf.compat.v1.Session(config=config) as sess:
        model = net(imgs, ados, REG_PENALTY=0, is_training=False)


        val_filename_queue = tf.compat.v1.train.string_input_producer(
            ["val_acc/Dan_res/val_dr_acc.tfrecords"], num_epochs=len(image_file_list)
        )
        ...
        print("Num of epoch:", len(image_file_list))
        for j in range(len(image_file_list)):
            model.load_trained_model(image_file_list[j], sess, is_imgs=True)
            model.load_trained_model(audio_file_list[j], sess, is_imgs=False)
            val_acc_list = []
            val_mean_acc_list = []
...
def val_acc():
    ...
    if (len(image_list) != PER_VAL * NUM_VAL):
        print("data length error!", len(image_list))
        return


    try:
        os.makedirs("val_acc/Dan_res")
    except OSError:
        print("Error: Creating val_acc/Dan_res")

    print("Beginning data to tf")
    val_filename = "val_acc/Dan_res/val_dr_acc.tfrecords"  # address to save the TFRecords file
